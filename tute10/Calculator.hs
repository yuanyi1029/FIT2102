...
-- Unexpected character: " "
-- >>> parse bracketExpr "((1 + 2) * 4))"
-- Times (Plus (Number 1) (Number 2)) (Number 4)
bracketExpr :: Parser Expr
bracketExpr =
  number <|> do
    _ <- is '('
    expr1 <- bracketExpr
    operator <- add <|> minus <|> times
    expr2 <- bracketExpr
    _ <- is ')'
...
multiplicationTerms = chain number times
...
numberOrParenthesizedExpr = number <|> parens parseNoBracketCalc
...
lowerPrecedenceExpr = chain higherPrecedenceExpr (add <|> minus)
...
higherPrecedenceExpr = chain numberOrParenthesizedExpr times
...
parseFullCalc = lowerPrecedenceExpr
